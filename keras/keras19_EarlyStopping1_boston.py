...
model.add(Dense(5, input_shape=(13,)))      # 다차원: input_shape 사용 (13, )
                                            # if (100, 10, 5)일 때, (10, 5)로 표현. 맨 앞의 100은 데이터 개수
...
EarlyStopping = EarlyStopping(monitor='val_loss',               # history의 val_loss 최소값 이용 
                              mode='min',                       # accuracy 사용 시에는 max로 설정 (정확도가 높을수록 좋기 때문에)
                              patience=10, 
                              restore_best_weights=True,
                              verbose=1)
hist = model.fit(x_train, y_train, epochs=300, batch_size=1, 
          validation_split=0.2, callbacks=[EarlyStopping],      # val_loss 기준으로 최소값이 n번 이상 갱신 안 되면 훈련 중지
